...
\begin{exercise}[Subspaces]
	...
	\]
	Prove that there is an isomorphism between \(L_\real^2\) and \(\real\). This
	allows us to identify \(L_\real^2\) with \(\real\), i.e. write \(L_\real^2=\real\).
	Show that \(\real\) and \(L_0^2\) are orthogonal in \(L^2\). Finally show,
	that
	\begin{equation}\label{eq: direct sum of linear subspaces}
		L^2 = \real + L_0^2 = \{ X + Y : X\in \real, Y\in L_0^2\}
	\end{equation}
\end{exercise}
...
\begin{exercise}[Projections]
	...
	\[
		\pi_\real(Y)=\pi_{L_\real^2}(Y) = \argmin_{X\in \real} \| X-Y\|^2.
	\]	
...
\begin{exercise}[Linear Model]
	...
	\[
		U = \real + \Span\{X_1,\dots, X_n\} = \Span\{\underbrace{X_0}_{:=1}, X_1,\dots, X_n\}
	\]
