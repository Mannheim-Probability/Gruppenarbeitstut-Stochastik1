...
\begin{exercise}[Zerlegung]
	...
	\(F\). Zeige dass es zwei Wahrscheinlichkeitsmaße \(\mu_d,\mu_s\) gibt, mit
	Verteilungsfunktionen \(F_d, F_s\), sodass \(F_s\) eine stetige Funktion ist,
	\[
		\mu_d = \sum_{k=1}^\infty a_k \delta_{x_k},
		\qquad (x_i)_{i\in\nat}, (a_i)_{i\in\nat} \subseteq \real
	\]
	und zuletzt gilt, dass
	\[
		\mu = c_d \mu_d + c_s \mu_s
	\]
	für \(c_d,c_s\in[0,1]\).
\end{exercise}
