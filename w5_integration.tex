...
über die wir sprechen wohldefiniert sind. So wie Fubini ``immer'' geht, geht
also auch das Vertauschen von Ableitung und Integral ``immer''. Die einzige
Stelle, an der das nicht ganz richtig ist, ist bei der ``lokalen Integrierbarkeit''
es genügt nicht, dass die Ableitung nur in \(x\) über \(\omega\) \(\mu\)-integrierbar
ist. Die resultierende Funktion in \(x\) muss friedlich genug sein, dass man
sie zumindest in einer Umgebung noch integrieren kann. Es genügt also, wenn
\[
	x\to \int \frac{\partial}{\partial t}f(x,\omega)d\mu(\omega)
\]
stetig in \(x\) ist, oder beschränkt, oder wenn uniform
\(\bigl|\frac{\partial}{\partial t}f(t,\omega)\bigr| \le g(\omega)\) für ein
integrierbares \(g\), oder wenn \(\frac{\partial}{\partial t}f(t,\omega)\)
monoton in \(t\) ist.
