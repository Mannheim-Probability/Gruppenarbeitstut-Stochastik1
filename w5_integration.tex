...
\begin{example}[Parameter Entfernen]
	Wir können uns mit der Substitutionsregel das berechnen der Verteilungsfunktion
	der allgemeinen Exponentialverteilung auf die kanonische Variante zurückführen:
	\[
		F_\lambda(t) = \int_0^t \underbrace{\lambda}_{=\frac{dy}{dx}}\exp(-\lambda x)dx
		\overset{y=\lambda x}=
		\int_0^{\lambda t} \exp(-y)dy
		= F_1(\lambda t).
	\]
	Ähnlich kann man eine allgemeine Normalverteilung leicht in \(\cN(0,1)\)
	überführen:
	\begin{align*}
		...
		\exp\left(-y/2\right)\underbrace{\frac{dy}{dx}}_{=\sigma}dy\\
